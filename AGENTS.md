...
| `participants-service` | Implémenté : CRUD, recherche, validation, authentification JWT, Prisma avec migration, tests Jest + Supertest, Dockerfile et healthcheck. Intégré à Compose, Nginx et la CI/CD. Tests et validation Prisma vérifiés localement le 11/08/2026. Build Docker non vérifié localement, moteur Docker indisponible. |
| `registrations-service` | Implémenté : inscription, annulation logique, listes, enrichissement REST, statistiques, contrôle des doublons et services amont, Prisma avec migration, tests Jest + Supertest, Dockerfile et healthcheck. Intégré à Compose, Nginx et la CI/CD. Tests et validation Prisma vérifiés localement le 11/08/2026. Le build Docker n'est pas vérifié localement, moteur Docker indisponible. L'inscription dépend encore de `GET /api/events/:id/availability`, non implémenté dans `event-service`. |
...
| `gateway/` | `nginx.conf` proxifie les quatre préfixes `/api/auth`, `/api/events`, `/api/participants` et `/api/registrations`. Le frontend reste intégré dans la même image (ADR 0005). |
| `docker-compose.yml` | Couvre les quatre services, leurs quatre bases, `gateway` (avec frontend intégré) et `uptime-kuma`. Télécharge les images GHCR (`ghcr.io/traorecheikh/eventis-<service>`), ne construit rien. Pour construire en local, ajouter `docker-compose.dev.yml` (`docker compose -f docker-compose.yml -f docker-compose.dev.yml up -d --build`). Configuration Compose validée le 10/08/2026. |
