...
| `docker-compose.yml` | Couvre les quatre services, leurs quatre bases, `gateway` (avec frontend intégré) et `uptime-kuma`. Télécharge les images GHCR (`ghcr.io/traorecheikh/eventis-<service>`), ne construit rien. Pour construire en local, ajouter `docker-compose.dev.yml` (`docker compose -f docker-compose.yml -f docker-compose.dev.yml up -d --build`). Les 9 conteneurs démarrent `healthy` (vérifié le 11/08/2026, migrations Prisma appliquées automatiquement au démarrage de chaque service) |
