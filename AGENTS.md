...
| `event-service` | Implémenté : `GET /events`, `POST /events`, `GET /events/:id`, `GET /events/:id/availability` (appelle `registrations-service`, 503 sans valeur optimiste si injoignable). Manquent encore `PUT /events/:id`, `DELETE /events/:id`, filtres `date`/`dateFrom`/`dateTo`/`location`. Voir le contrat `knowledge-base/api/events-service.md`. Le dossier s'appelle `event-service` (singulier) alors que le contrat et le README disent `events-service` : renommage à faire dans une PR dédiée, pas en même temps qu'une autre modification |
| `participants-service` | Implémenté : CRUD, recherche, validation, authentification JWT, Prisma avec migration, documentation OpenAPI générée (`/api/participants/docs`), tests Jest + Supertest, Dockerfile et healthcheck. Intégré à Compose, Nginx et la CI/CD. Flux complet vérifié le 11/08/2026 via `docker compose up` réel (voir plus bas) |
| `registrations-service` | Implémenté : inscription, annulation logique, listes, enrichissement REST, statistiques, contrôle des doublons et services amont, Prisma avec migration, documentation OpenAPI générée (`/api/registrations/docs`), tests Jest + Supertest, Dockerfile et healthcheck. Intégré à Compose, Nginx et la CI/CD. Flux complet vérifié le 11/08/2026 : inscription, annulation, réinscription, refus de surbooking (409 EVENT_FULL), tout testé pour de vrai via `docker compose up` et des appels HTTP à travers la passerelle |
| `frontend/` | Implémenté : authentification (inscription avec choix du rôle, connexion, session JWT, garde de routes), évenements (liste paginée, détail, disponibilité en temps réel, création), participants (création et modification de profil), inscriptions (inscription liée à un profil participant existant, annulation, liste "Mes inscriptions"), tableau de bord (statistiques réelles via `GET /registrations/stats`). Stack conforme à la section 1 (Vue 3, Vite 8, Pinia, Tailwind, Reka UI, lucide, vue-sonner, @vueuse/motion). Aucune donnée de repli locale : une erreur backend reste visible, jamais masquée. 80 tests Vitest et 7 scénarios Playwright (contre la pile complète, sans mock), tous verts. Vérifié le 13/08/2026 par un `docker compose up` réel et un parcours complet (inscription organisateur, création d'événement, inscription participant, création de profil, inscription à l'événement, annulation), captures d'écran incluses dans la revue de PR |
| `gateway/` | `nginx.conf` proxifie les quatre préfixes `/api/auth`, `/api/events`, `/api/participants` et `/api/registrations`. Le frontend reste intégré dans la même image (ADR 0005). Vérifié le 11/08/2026 : les 4 préfixes répondent, `/` sert le frontend |
| `docker-compose.yml` | Couvre les quatre services, leurs quatre bases, `gateway` (avec frontend intégré) et `uptime-kuma`. Télécharge les images GHCR (`ghcr.io/traorecheikh/eventis-<service>`), ne construit rien. Pour construire en local, ajouter `docker-compose.dev.yml` (`docker compose -f docker-compose.yml -f docker-compose.dev.yml up -d --build`). Les 10 conteneurs démarrent `healthy`, `uptime-kuma` inclus (vérifié le 11/08/2026, migrations Prisma appliquées automatiquement au démarrage de chaque service) |
| `.github/workflows/ci.yml` | Fait : jobs indépendants par service (path filters sur `auth-service/**`, `event-service/**`, `participants-service/**`, `registrations-service/**`, `frontend/**`, `gateway/**`), `lint:esm` puis `test` par service backend (base Postgres éphémère en conteneur de service GitHub Actions). Job `frontend` : ESLint, Vitest, génération OpenAPI/Orval, build. Job `e2e` : construit et démarre la pile complète (`docker compose up`) puis rejoue les scénarios Playwright contre l'application réelle, pas encore ajouté aux checks requis (job plus lourd, à surveiller avant de le rendre bloquant). Vérifié par une execution réelle sur GitHub (tous les jobs verts) |
| `.github/workflows/cd.yml` | Fait : sur push `main`, rejoue les tests, construit et pousse les 5 images (`auth-service`, `event-service`, `participants-service`, `registrations-service`, `gateway`) sur GHCR avec deux tags (`latest` et le SHA du commit, pour permettre un rollback ciblé, voir R-03 dans `knowledge-base/scrum/risques.md`), puis appelle le webhook Dokploy. Les images GHCR sont **privées par défaut** : suivre `knowledge-base/runbooks/installer-vps-dokploy.md` section 6 (les rendre publiques, ou configurer un registry privé dans Dokploy) avant le premier déploiement, sinon Dokploy ne pourra pas télécharger les images. Structure vérifiée (`docker build` local des 5 images), le job `deploy` (webhook) n'a pas pu être exécuté pour de vrai avec les 2 nouvelles images : nécessite un vrai push sur `main` |
